...
public class AudioPathCreator : AssetPostprocessor {
	...
	[MenuItem("Tools/AudioManager/Create BGM Path")]
	private static void CreateBGMPath() {
		Create(BGM_DIRECTORY_PATH);
	}

	//SEファイルへのパスを定数で管理するクラスを作成
	[MenuItem("Tools/AudioManager/Create SE Path")]
	...
	private static void Create(string directoryPath) {
        ...
        //定数クラス作成
        ConstantsClassCreator.Create(directoryName + "Path", directoryName + "ファイルへのパスを定数で管理するクラス", audioPathDict, exportPath, "AudioManager");
	}
